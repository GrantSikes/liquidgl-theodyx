...
(function () {
  ...
  function selfBase() {
    ...
    if (!src) { var ss = document.getElementsByTagName('script'); for (var i = ss.length - 1; i >= 0; i--) { if (/theodyx-home-fx\.js/.test(ss[i].src || '')) { src = ss[i].src; break; } } }
    if (src && /theodyx-home-fx\.js/.test(src)) return src.replace(/theodyx-home-fx\.js.*$/, '');
  ...
  /* ── module 1: media mosaic, contract-mounted ───────────────────────── */
  function mountMosaic() {
    var m = document.querySelector('[data-thx-anim="mosaic"]');
    if (!m || m.__thxFxMounted) return; m.__thxFxMounted = true;
    var img = m.querySelector('img');
    var reduce = false; try { reduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches; } catch (e) {}
    var mw = (m.getAttribute('data-mosaic-word') || '').trim(); if (mw) WORD = mw;
    var mm = (m.getAttribute('data-mosaic-media') || '').trim();
    if (mm) { var ml = mm.split(',').map(function (u) { return u.trim(); }).filter(Boolean); if (ml.length) MEDIA = ml; }
    if (reduce) { if (img) img.style.opacity = '1'; m.__thxMode = 'reduced'; return; }
    Promise.all(MEDIA.map(loadImg)).then(function (imgs) {
      var atlas = buildAtlas(imgs);
      if (!atlas) { if (img) img.style.opacity = '1'; m.__thxMode = 'static'; return; }
      var canvas = document.createElement('canvas'); canvas.setAttribute('aria-hidden', 'true'); m.appendChild(canvas);
    ...
    function swap() { try { var f = canvas.cloneNode(false); m.replaceChild(f, canvas); canvas = f; } catch (e) {} }
    function build() {
      if (inst) { go(); return; }
      try { inst = makeHero(canvas, atlas); } catch (e) { inst = null; }
      if (inst) { m.__thxMode = 'webgl'; go(); return; }
      if (tries < 14) { tries++; swap(); setTimeout(build, 500); return; }
      img.style.opacity = '1'; m.__thxMode = 'static';   // WebGL never came up -> show the still image
    }
  ...
  /* ── module 2: hero video controls (native video + native buttons) ──── */
