...
(async () => {
  ...
  for (const [label, ctxOpts] of [['desktop', {}], ['phone', { ...devices['iPhone 13'] }]]) {
    ...
    check('LAND-01 (' + label + '): first viewport shows the offer — h1 + CTA, no full-screen overlay, form visible, safety section + required acknowledgement present', s.h1InView && s.cta && !s.overlay && s.formVisible && s.safety && s.ack && s.inert === 0, s);
