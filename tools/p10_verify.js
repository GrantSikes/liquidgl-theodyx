...
(async () => {
  ...
  for (const [label, ctxOpts] of [['desktop', {}], ['phone', { ...devices['iPhone 13'] }]]) {
    ...
    check('LAND-01 (' + label + '): first viewport carries the offer (h1 + CTA) behind the gate; form hidden and page inert until it is answered (scouting 2.4.0: the safety stage first, the statement is the gate, not a section)', s.h1InView && s.cta && (s.overlay === 'sc-gate-safety' || s.overlay === 'sc-gate-age') && !s.formVisible && s.inert > 0, s);
