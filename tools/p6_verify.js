...
(async () => {
  ...
  for (const p of pages) {
    ...
    if (p === '/scouting') { /* owner 2026-09-05: the age dialog is back - answer it so the form text is comparable with the baseline */ try { await pg.fill('#sc-gate-dob', (new Date().getFullYear() - 22) + '-06-15'); await pg.click('#sc-gate-age-go'); } catch (e) {} await pg.waitForTimeout(900); }
