...
(async () => {
  ...
  for (const p of pages) {
    ...
    const before = fs.existsSync(beforePath) ? JSON.parse(fs.readFileSync(beforePath, 'utf8')) : null; if (before && before.text) before.text = before.text.replace(/ LANGUAGE\b/g, ''); /* round 4: the footer language row is gone by owner decision */
  ...
  { const ctx = await b.newContext({ viewport: { width: 1280, height: 900 } }); const pg = await ctx.newPage();
    ...
    /* round 4 (owner, 2026-09-06): the language switcher is retired, so the 3-locale render check is gone with it */
