// Phase 9 accessibility verifier. Usage: node p9_verify.js [--prod]  (staging nhq.webflow.io by default; fresh context per page)
...
(async () => {
  ...
  { const { page, ctx, errors } = await open('/our-capabilities');
    ...
    const plates = await page.evaluate(() => [...document.querySelectorAll('#thx-nav .thx-nav-menu a')].map(a => a.style.getPropertyValue('--thx-plate') || '0'));
    check('C-01: per-word plates engage over the dark band at scrollY 1100 (nav 4.7.0 live)', plates.some(p => parseFloat(p) >= 0.3), plates);
  ...
  { const { page, ctx, errors } = await open('/scouting', { settle: 2500 });
    const g = await page.evaluate(() => { const gate = document.getElementById('sc-gate-safety'); return { focusIn: !!(gate && gate.contains(document.activeElement)), inert: [...document.body.children].filter(e => e.hasAttribute('inert')).length, others: [...document.body.children].filter(e => !e.contains(gate) && e.tagName !== 'SCRIPT').length }; });
    let escapes = 0; for (let i = 0; i < 12; i++) { await page.keyboard.press('Tab'); const inside = await page.evaluate(() => { const gate = document.getElementById('sc-gate-safety'); return !!(gate && gate.contains(document.activeElement)); }); if (!inside) escapes++; }
    check('KB-01/SEM-02 (P0): safety gate takes focus, traps Tab, rest of page inert', g.focusIn && escapes === 0 && g.inert > 0, { ...g, escapes });
    const f = await page.evaluate(() => ({ required: ['sc-email', 'sc-firstName', 'sc-lastName', 'sc-dob', 'sc-platform', 'sc-country', 'sc-consent'].map(id => { const e = document.getElementById(id); return e ? e.required : null; }), bday: (document.getElementById('sc-dob') || {}).getAttribute && document.getElementById('sc-dob').getAttribute('autocomplete'), country: document.getElementById('sc-country') && document.getElementById('sc-country').getAttribute('autocomplete'), rep: (() => { const r = document.querySelector('#sc-rep .sc-chips'); return r ? r.getAttribute('role') + '/' + r.getAttribute('aria-labelledby') : null; })(), drop: (() => { const d = document.getElementById('sc-mediakit'); return d ? d.tagName : null; })() }));
    ...
    await ctx.close(); }
  // 5. article template
  { const { page, ctx, errors } = await open('/our-thinking/remove-the-creators-and-all-media-platforms-go-dark', { settle: 2500 });
  ...
  { const { page, ctx } = await open('/our-thinking/the-end-of-the-sponsorship-era', { settle: 2500 });
    ...
    await ctx.close(); }
  // 7. index (ethos) page
  { const { page, ctx, errors } = await open('/index/planning-for-creators-brands-and-beyond', { settle: 2500 });
    ...
    await ctx.close(); }
  // 8. policy page
  { const { page, ctx, errors } = await open('/policies/privacy-policy');
    ...
    await ctx.close(); }
  // 9. contact
  { const { page, ctx } = await open('/contact', { settle: 3500 });
    ...
    await ctx.close(); }
  // 10. hub
  { const { page, ctx, errors } = await open('/our-thinking');
  ...
  { const { page, ctx } = await open('/about', { ctx: { ...devices['iPhone 13'], viewport: { width: 360, height: 780 } } });
    ...
    await ctx.close(); }
  { const { page, ctx } = await open('/our-thinking', { ctx: { ...devices['iPhone 13'] } });
  ...
  fs.writeFileSync(out, JSON.stringify({ base: BASE, results, fails }, null, 1));
